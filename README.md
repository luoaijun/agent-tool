...
**Windows 桌面 AI 多模型任务运行器 · 多模型并行 · 本地语音 · 跨设备联动**
...
Agent CLI 是一款 Windows 桌面 AI 工具，支持同时运行多个 AI 模型（DeepSeek、OpenAI、Claude、Gemini 等），完成任务的分解、并行执行和结果汇总。内置本地语音引擎、微信小程序授权，让你以最自然的方式与 AI 协作。
...
| 🎙️ **本地语音** | Windows 原生语音引擎，无需联网即可语音输入和语音播报 |
...
- 本地语音引擎：Windows 原生 STT/TTS，无需外部 API
