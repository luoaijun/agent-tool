...
**Windows 桌面 AI 多模型任务运行器 · 多模型并行 · 本地语音（实验性） · 本地图片生成 · 跨设备联动**
...
Agent CLI 是一款 Windows 桌面 AI 工具，支持同时运行多个 AI 模型（DeepSeek、小米 MiMo 等兼容 OpenAI 接口的模型），完成任务的分解、并行执行和结果汇总。内置本地语音引擎（实验性）、微信小程序授权，让你以最自然的方式与 AI 协作。
...
|  **本地图片生成** | 支持下载 Stable Diffusion 等离线模型，本地生成高质量图片，无需联网、无需 API 费用 |
...
Agent CLI 需要登录后才能使用，账号密码通过微信小程序生成：

1. **登录小程序**：微信搜索小程序 **「我和猫猫cli」**，使用微信授权登录
2. **生成凭证**：在小程序内进入「我的」→「CLI 登录凭证」，系统自动生成专属用户名和密码
3. **登录桌面端**：打开 Agent CLI，使用小程序生成的账号密码登录即可

> ⚠️ 请妥善保管你的登录凭证，不要泄露给他人。
...
也支持任何兼容 OpenAI API 接口的自定义模型服务（如 OpenAI、Claude、Gemini 等，需自行配置）。
...
- 设置页面布局改进
- 搜索自动展示最近对话
- 本地图片生成：支持下载 Stable Diffusion 离线模型，本地生成高质量图片
